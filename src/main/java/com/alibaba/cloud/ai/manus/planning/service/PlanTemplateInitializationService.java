...
/**
 * Plan template initialization service for managing plan template configurations with multi-language support
 */
@Service
public class PlanTemplateInitializationService {
    ...
    public void initializePlanTemplatesForNamespaceWithLanguage(String namespace, String language) {
        try {
            ...
            for (String planName : planNames) {
                try {
                    ...
                    if (planTemplateId != null) {
                        log.debug("Successfully initialized plan template: {} -> planTemplateId: {}", planName, planTemplateId);
                    } else {
                        log.warn("Failed to initialize plan template: {} - no planTemplateId returned", planName);
                    }
                } catch (Exception e) {
                    log.error("Failed to initialize plan template: {} for namespace: {} with language: {}", 
                            planName, namespace, language, e);
            ...
            log.error("Failed to initialize plan templates for namespace: {} with language: {}", namespace, language, e);
            throw e;
        }
    }
    ...
    public void resetAllPlanTemplatesToLanguage(String namespace, String language) {
        ...
        for (PlanTemplate template : existingTemplates) {
            List<PlanTemplateVersion> versions = planTemplateVersionRepository.findByPlanTemplateIdOrderByVersionIndexAsc(template.getPlanTemplateId());
        ...
        try {
            ...
            try {
                ...
                if (planTemplateId == null || planTemplateId.trim().isEmpty()) {
                    throw new RuntimeException("planTemplateId is required in JSON configuration for plan: " + planName);
                }
            ...
            log.info("Successfully initialized plan template: {} (language: {}) -> planTemplateId: {}", 
                    planName, language, executionPlan.getPlanTemplateId());
     ...
     */
    @Transactional
    public void savePlanTemplate(String planTemplateId, String title, String userRequest, String planJson, boolean isInternalToolcall) {
        try {
    ...
    /**
     * Initialize plan templates for a specific language (automatically finds all plan names)
        ...
        for (String planName : planNames) {
            try {
                ...
                if (planTemplateId != null) {
                    ...
                    log.info("Successfully initialized plan template: {} (language: {}) -> planTemplateId: {}", planName, language, planTemplateId);
                } else {
    ...
    public List<String> scanAvailablePlansForLanguage(String language) {
        try {
            ...
            log.info("Scanned {} plan template configurations for language {}: {}", planList.size(), language, planList);
            return planList;
        } catch (Exception e) {
